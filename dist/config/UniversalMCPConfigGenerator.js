...
export class UniversalMCPConfigGenerator {
    ...
    async getPackageSearchPaths() {
        ...
        switch (this.platform) {
            ...
            case 'darwin':
                // macOS paths
                paths.push(path.join('/usr/local/lib/node_modules', packageName), path.join(this.homeDir, '.npm-global', 'lib', 'node_modules', packageName), path.join('/opt/homebrew/lib/node_modules', packageName));
                break;
            case 'linux':
                // Linux paths
                paths.push(path.join('/usr/lib/node_modules', packageName), path.join('/usr/local/lib/node_modules', packageName), path.join(this.homeDir, '.npm-global', 'lib', 'node_modules', packageName), path.join(this.homeDir, '.local', 'lib', 'node_modules', packageName));
                break;
        }
        return paths;
    }
     ...
     */
    async getClaudeDesktopLocations() {
        const locations = [];
        switch (this.platform) {
            case 'win32':
                if (process.env.APPDATA) {
     ...
     */
    generateStandardMCPConfig(options = {}) {
        if (!this.nodePath || !this.packagePath) {
            throw new Error('Not initialized. Call initialize() first.');
        }
        const serverName = options.serverName || 'castplan-ultimate';
     ...
     */
    async installConfig(location, options = {}, backupExisting = true) {
...
//# sourceMappingURL=UniversalMCPConfigGenerator.js.map