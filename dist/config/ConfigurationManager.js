...
export class ConfigurationManager {
    ...
    async generateConfiguration() {
        ...
        const [projectAnalysis, packageManagerInfo, platformInfo] = await Promise.all([
        ...
        this.claudeConfigGenerator = new ClaudeDesktopConfigGenerator(projectAnalysis.projectInfo, platformInfo, this.logger);
        this.standardMCPGenerator = new StandardMCPConfigGenerator(projectAnalysis.projectInfo, platformInfo, this.logger);
     ...
     */
    convertPackageManagerInfo(detectionResult) {
        const availableManagers = detectionResult.managers
        ...
        const allManagers = ['npm', 'yarn', 'pnpm', 'uv', 'uvx', 'pip'];
     ...
     */
    async generateRuntimeConfig(projectInfo, envDefaults) {
        ...
        return {
            ...
            ai: {
                ...
                provider: envDefaults[`${prefix}_AI_PROVIDER`] || 'openai',
            ...
            logging: {
                level: envDefaults[`${prefix}_LOG_LEVEL`] || 'info',
     ...
     */
    extractPrefixFromDefaults(envDefaults) {
    ...
    async generateClaudeDesktopConfig(options = {}) {
        ...
        return await this.claudeConfigGenerator.generateConfiguration(this._config.environment, options);
    }
    /**
     ...
     */
    async generateEnvironmentFiles(target) {
        if (!this._config) {
            throw new Error('Configuration not initialized. Call initialize() first.');
        }
     ...
     */
    async writeEnvironmentFiles(outputDir, target) {
    ...
    async generateMCPConfigFiles(options = {}) {
        ...
        return this.standardMCPGenerator.generateConfigFiles(this._config.environment, options);
    }
    /**
    ...
    async installMCPConfigs(options = {}) {
        ...
        const targetFiles = options.clients
            ? configFiles.filter(file => options.clients.some(client => file.name.toLowerCase().includes(client.toLowerCase())))
            : configFiles;
        const success = [];
        const failed = [];
    ...
    generateMCPConfigTemplate(options = {}) {
        ...
        return this.standardMCPGenerator.generateConfigurationTemplate(this._config.environment, options);
    }
    /**
