...
export class PackageManagerDetector {
    ...
    async detectAvailableManagers() {
        ...
        // Detect each manager
        const detectionPromises = managers.map(manager => this.detectSingleManager(manager));
     ...
     */
    async detectSingleManager(managerConfig) {
     ...
     */
    async getManagerVersion(executablePath) {
        return new Promise((resolve) => {
    ...
    async installPackage(packageName, options = {}) {
        const { global = true, preferManager, timeout = 5 * 60 * 1000, // 5 minutes
        retries = 2, fallbackChain = ['npm', 'yarn', 'pnpm', 'bun'] } = options;
        ...
        const orderedManagers = this.orderManagersByPreference(detection.managers, preferManager, fallbackChain);
        let lastError;
        ...
        for (const manager of orderedManagers) {
            ...
            for (let attempt = 1; attempt <= retries; attempt++) {
                try {
                    ...
                    const result = await this.attemptInstallation(manager, packageName, global, timeout);
                    if (result.success) {
                        return {
                            ...result,
     ...
     * Order managers by preference
     */
    orderManagersByPreference(managers, preferManager, fallbackChain = []) {
     ...
     */
    async attemptInstallation(manager, packageName, global, timeout) {
        return new Promise((resolve) => {
            // Build command
     ...
     */
    async isPackageInstalled(packageName, global = true) {
        const detection = await this.detectAvailableManagers();
     ...
     */
    async checkPackageWithManager(manager, packageName, global) {
        return new Promise((resolve) => {
            let args;
            switch (manager.name) {
                case 'npm':
