...
export class DateTimeService implements IDateTimeService {
   ...
   */
  formatDate(date: string, format: 'iso' | 'localized' | 'short' = 'iso'): string {
  ...
  private formatWithTimezone(date: Date, options: Intl.DateTimeFormatOptions): string {
   ...
   */
  getWorkDay(daysOffset: number = 0): string {
    const config = this.i18nService.getConfig();
    let date: Date;
   ...
   */
  createDocumentHeader(title: string, author: string = 'CastPlan Team'): string {
...
`;
   ...
   */
  getTimezoneInfo(): { name: string; offset: string; current: string } {
    const now = new Date();
    const config = this.i18nService.getConfig();
    return {
