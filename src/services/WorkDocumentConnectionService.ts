...
import {
  WorkDocumentConnectionService as IWorkDocumentConnectionService,
  WorkDocumentConnection
 ...
 * Created: 2025-07-29
 */
export class WorkDocumentConnectionService implements IWorkDocumentConnectionService {
  private db: any;
  ...
  async createConnection(connection: Omit<WorkDocumentConnection, 'id' | 'createdAt' | 'updatedAt'>): Promise<WorkDocumentConnection> {
    try {
      await this.ensureInitialized();
      const now = new Date().toISOString();
      const newConnection: WorkDocumentConnection = {
        id: uuidv4(),
        ...connection,
        createdAt: now,
        updatedAt: now
  ...
  async findConnectionsByWork(workType: string, filePaths: string[]): Promise<WorkDocumentConnection[]> {
    try {
      await this.ensureInitialized();
      ...
      return rows
        ...
        .filter((connection: WorkDocumentConnection) => {
          const connectionPaths = connection.filePaths;
          return filePaths.some(path => 
            connectionPaths.some((connPath: string) => 
              path.includes(connPath) || connPath.includes(path)
            )
          );
        });
    } catch (error) {
  ...
  async updateConnectionStrength(id: string, strength: number): Promise<void> {
    try {
      await this.ensureInitialized();
      const now = new Date().toISOString();
  ...
  async getAllConnections(): Promise<WorkDocumentConnection[]> {
    try {
      await this.ensureInitialized();
      const rows = await this.db.all(`
        SELECT * FROM work_connections 
  ...
  private mapRowToConnection(row: any): WorkDocumentConnection {
    return {
      id: row.id,
