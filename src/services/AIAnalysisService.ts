...
import {
  AIAnalysisService as IAIAnalysisService,
  AIAnalysis
...
export class AIAnalysisService implements IAIAnalysisService {
  ...
  private aiClient: any; // Will be initialized based on provider
   ...
   */
  private initializeAIClient(OpenAIClass?: any): void {
    try {
   ...
   */
  async analyzeQuality(documentPath: string): Promise<AIAnalysis> {
    try {
   ...
   */
  async detectDuplicates(documentPath: string): Promise<AIAnalysis> {
    try {
      const content = await this.readDocumentContent(documentPath);
   ...
   */
  async calculateRelevance(documentPath: string, workContext: string): Promise<AIAnalysis> {
    try {
      const content = await this.readDocumentContent(documentPath);
   ...
   */
  async generateInsights(documentPath: string): Promise<string[]> {
    try {
      const content = await this.readDocumentContent(documentPath);
  ...
  private async analyzeQualityWithOpenAI(documentPath: string, content: string): Promise<AIAnalysis> {
    try {
...
Format as JSON with fields: score, insights, suggestions, confidence`;
  ...
  private async calculateRelevanceLocally(documentPath: string, content: string, workContext: string): Promise<AIAnalysis> {
    const insights: string[] = [];
    const suggestions: string[] = [];
  ...
  private async detectDuplicatesWithOpenAI(documentPath: string, content: string): Promise<AIAnalysis> {
    try {
...
Format your response as JSON with fields: score, insights, suggestions, confidence`;
  ...
  private async generateDuplicateInsights(content: string, score: number): Promise<string[]> {
    const insights: string[] = [];
  ...
  private generateDuplicateSuggestions(score: number): string[] {
    const suggestions: string[] = [];
  ...
  private findRepeatedPatterns(content: string): string[] {
  ...
  private async generateInsightsWithOpenAI(documentPath: string, content: string): Promise<string[]> {
    try {
