...
import { EnvironmentConfig, ProjectInfo, RuntimeConfig } from './types.js';
...
export class EnvGenerator {
   ...
   */
  private generatePrefix(customPrefix?: string): string {
    if (customPrefix) {
   ...
   */
  private generateEnvironmentVariables(prefix: string, options: EnvGenerationOptions): Record<string, string | undefined> {
   ...
   */
  private generateDefaultValues(prefix: string, options: EnvGenerationOptions): Record<string, string> {
   ...
   */
  private generateWatchPatterns(): string[] {
