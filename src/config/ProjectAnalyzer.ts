...
export class ProjectAnalyzer {
   ...
   */
  private async detectPackageManagers(): Promise<PackageManagerType[]> {
   ...
   */
  private async detectDocumentationPatterns(): Promise<string[]> {
   ...
   */
  private async generateRecommendedCategories(): Promise<string[]> {
   ...
   */
  private async detectProjectVersion(): Promise<string | undefined> {
    // Try package.json
    try {
      const packageJsonPath = path.join(this.projectRoot, 'package.json');
      const packageJson = JSON.parse(await fs.readFile(packageJsonPath, 'utf8'));
   ...
   */
  private async detectProjectDescription(): Promise<string | undefined> {
    // Try package.json
    try {
      const packageJsonPath = path.join(this.projectRoot, 'package.json');
      const packageJson = JSON.parse(await fs.readFile(packageJsonPath, 'utf8'));
  ...
  private calculateConfidence(
    ...
    if (projectInfo.type !== 'unknown') confidence += 0.3;
    if (projectInfo.framework) confidence += 0.2;
    if (projectInfo.version) confidence += 0.1;
    if (projectInfo.description) confidence += 0.1;

    // Framework detection confidence
    if (frameworks.length > 0) confidence += 0.2;
    if (frameworks.length > 1) confidence += 0.1;

    // Package manager detection confidence
    if (packageManagers.length > 0) confidence += 0.1;
